...
for i, line in enumerate(lines):
    if "view === 'copy'" in line and "Lançamento Canvas" in lines[i+4]: # heuristic
        start_idx = i - 1 # include the comment
    if "{view === 'reports' && (" in line:
        end_idx = i - 1 # up to the reports block
        break

if start_idx != -1 and end_idx != -1:
    new_content = """        {/* ── COPIA DE ATIVIDADES ─────────────────────────────────────────────────── */}
        {view === 'copy' && <>
